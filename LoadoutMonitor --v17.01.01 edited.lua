			...
			if players then
				...
				for k,player in pairs(players) do
					...
					for k,weapon_type in pairs({"Melee","Range"}) do
						--local line = {"",""}
						local line = {""} -- remember to access as array
						local traits_text = {perk = {"",""},blessing = {"",""}}
						
						-- For each player, get perks and blessings for melee and ranged weapons
						if human then
						...
						--[[
						-- row_scoreboard_weapon (melee/range) 1, 2 is the blank space
						for i =1,2 do
						...
						-- Write weapon name to scoreboard
						scoreboard:update_stat(string.format("row_scoreboard_weapon_%s_%s",weapon_type,1), account_id, line[1])
						-- Write blessings to scoreboard
						local blessings_line = {""}
						blessings_line[1] = blessings_line[1] .. traits_text.blessing[1] .. " " .. traits_text.blessing[2]
						scoreboard:update_stat(string.format("row_scoreboard_weapon_%s_blessing_%s",weapon_type,1), account_id, blessings_line[1])
